...
| V2 | 对话式 Test Agent：Agent Core + 持续对话 + TestArtifact（Node/Revision/Operation/Diff/Undo）+ Artifact Tools + Chat/MindMap/Diff 工作台 | V2-P01～P04（消息/Provider/Agent Loop/会话持久化）已通过各自范围验收；P05 起按 [V2 新路线](V2/12_POST_P04_DEVELOPMENT_PLAN.md) 实施，尚未开始 |
...
- 2026-09-04 源码审计：conversation Agent Loop（loop/tool_executor/policy/budget/messages/events）已实现但未接任何 Worker/Router/Service，当前不存在 ConversationRunner；conversation queued Run 不被 Worker 消费；TestArtifact / Revision / Diff 尚不存在。P05 起点详见 V2/01_AGENT_DEVELOPMENT_PLAN.md 与 V2/12_POST_P04_DEVELOPMENT_PLAN.md。
...
docs/
  ...
  V2/
    README.md
    01_AGENT_DEVELOPMENT_PLAN.md        V2-P01～P10 执行计划（P05 起按 12）
    02_DEVELOPMENT_RECORD.md            新路线事实记录
    03_ACCEPTANCE_CHECKLIST.md          P01～P04 记录 + 新 P05～P10 门禁
    04_CONVERSATIONAL_TEST_AGENT_PRD.md 对话式 Test Agent 产品定义
    ...
    10_PI_SOURCE_AUDIT.md               固定源码、符号、测试及差异（Agent Core）
    11_V2_DIRECTION_ADR.md              固定 Workflow → 对话式 Test Agent 决策
    12_POST_P04_DEVELOPMENT_PLAN.md     P05～P10 新路线任务书
    13_TEST_ARTIFACT_CASE_TREE_DESIGN.md Artifact 数据模型
    14_ARTIFACT_TOOL_AND_AGENT_PROTOCOL.md Agent × Artifact 工具协议
    15_CHAT_MINDMAP_DIFF_PRODUCT_DESIGN.md 工作台产品设计
    16_LEGACY_WORKFLOW_MIGRATION_PLAN.md Legacy Workflow 退役计划
    17_EXISTING_DOCS_CHANGESET.md       本次既有文档修改记录（已执行）
    04_CASE_GENERATION_AGENT_PRD.md / 05_CASE_GENERATION_AGENT_TECHNICAL_DESIGN.md
                                        历史跳转说明（不再指向 V3.1）
  ...
  V3/
    ...
    04_CASE_GENERATION_AGENT_PRD.md     已进入 V2 的说明
    05_CASE_GENERATION_AGENT_TECHNICAL_DESIGN.md  同上
  archive/PRE_PI_V2_2026-09-03/         旧路线完整快照
~~~

V2 内旧 04_CASE_GENERATION... / 05_CASE_GENERATION... 文件仅保留跳转说明，指向 V2 Artifact 路线与归档，不能再作为当前 V2/V3 任务书。
...
8. 业务任务延后不等于删掉已实现业务；V2 的 test-design Skill（P08 起）按需加载，Skill/catalog 不默认自动执行固定业务流程；固定 case_generation Workflow 按 [V2/16](V2/16_LEGACY_WORKFLOW_MIGRATION_PLAN.md) 退役，不隐藏在新 Tool 内部继续整链调用。
...
| 2026-09-04 | V2 转为“对话式 Test Agent”：持续对话 + 可持续编辑 TestArtifact；基础测试设计与用例协作编辑进入 V2；固定 case_generation Workflow 退役 | ADR 见 V2/11_V2_DIRECTION_ADR.md；P05～P10 见 V2/12；退役见 V2/16 |
