...
| V2.1-T08.2 | 模型空响应可恢复处理与失败诊断（analyze_and_plan 空内容不再永久失败） | 已完成 | 2026-09-03 | Gateway 空内容有界重试 + 失败步骤落诊断 + 前端提示；见第 20 节 |
...
## 20. 2026-09-03 — 模型空响应可恢复处理与失败诊断（真实环境 analyze_and_plan 空内容）

### 现象与证据

正式环境出现“微信1v1聊天功能”需求生成失败：两次发送各自成一个 Run。真实 MySQL 只读核查：

- run 1（11:54）`analyze_and_plan` 失败，`业务场景未配置模型: requirement_to_function_case`——当时该场景 `llm_scene_configs.model_id` 未绑定（环境配置问题，随后用户已配置，api_doc 等其他场景仍为 None）；
- run 2（12:16）同一失败步骤，报 `模型返回内容为空，无法解析为 JSON。`——DeepSeek（provider_type=deepseek → OpenAI-compatible Adapter）返回 HTTP 200 但 `choices[0].message.content` 为空，`_parse_json_content` 抛 `LLMOutputValidationError`（retryable=False），workflow 对第一次 LLM（analyze_and_plan）无兜底 → 永久失败；且失败步骤 provider/model/finish_reason/request_id 全部未落库，无法判断是瞬时抖动、推理截断(finish_reason=length)还是内容安全拦截。

### 修改

- `backend/app/services/llm_gateway.py`：`complete()` 在连接层重试之外，把“HTTP 200 但无 content 且无 tool_calls”识别为**可恢复瞬时异常**（`LLMProviderError(llm_empty_content, retryable=True)`，有界重试 2 次指数退避）；重试耗尽后最终失败并携带 provider/model/finish_reason/request_id 诊断属性。合法 tool_calls（content 空但有工具调用）不被误判。
- `backend/app/agents/runtime/runner.py`：`_fail_step_exception` 读取异常上的 provider_name/model_name/finish_reason/request_id/error_code，写入失败 AgentStep 的 provider/model 列与 output_json 诊断；普通异常行为不变（无这些属性时 output_json 仍为 None）。
- `frontend/src/components/test-agent/TestAgentWidget.jsx`：run 失败且错误信息命中“返回内容为空/无法解析为 JSON/未通过 Schema/超时/限流”时，在会话中追加可操作提示（可重试一次；连续多次请检查场景绑定的模型、API Key 与配额）。

### 实际验证

```text
backend（项目 venv）:
pytest tests/services/test_llm_gateway.py tests/agents/test_agent_runtime.py -q
36 passed
pytest tests/workers tests/agents tests/api/test_agent_api.py tests/services/test_agent_platform_services.py tests/services/test_llm_gateway.py -q
261 passed

frontend:
npx eslint src/components/test-agent/TestAgentWidget.jsx → exit 0
```

新增用例：空内容→重试→第二次成功（退避 1 次）；空内容连续 3 次→最终失败且 error_code=llm_empty_content、provider/model/finish_reason/request_id 完整；content 空但带 tool_calls 不重试；Runner 把带诊断属性的 LLM 异常写入失败步骤 output_json。全程 Fake 适配器/回放，零真实 LLM、零网络。

### 边界/待定

- 仍无法自动区分“瞬时空返回 / finish_reason=length 推理截断 / 内容安全拦截”，需上游供应商诊断；本次只保证失败可恢复、可观测、可操作。
- `api_doc_to_api_case` 等场景 `model_id=None`：从接口文档“交给 Agent”前需先在模型管理中为场景绑定模型，否则会再撞上“业务场景未配置模型”。
