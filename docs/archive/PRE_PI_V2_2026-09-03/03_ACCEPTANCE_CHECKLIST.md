...
- [x] 模型返回 HTTP 200 但空内容时按可恢复瞬时异常有界重试，重试耗尽后失败步骤携带 provider/model/finish_reason/request_id 诊断，前端给出重试/查配置提示。（2026-09-03 V2.1-T08.2：36 条 + 261 条回归；Fake 适配器回放，见 02 开发记录第 20 节）
