...
- **摘要覆盖边界 = 逻辑位置（rank）**：物理 sequence_no 在 follow-up 插队时会与逻辑序错位（如 B 的用户行先落库、A 的助手行后落库），物理 marker 会把“逻辑上未覆盖”的用户消息误判为已覆盖而永久丢失。因此列/字段名为 **through_visible_rank**（run-bounded 可见逻辑序 rank，不再用 sequence_no 命名掩盖语义），并配 immutable **through_message_id** 锚点（边界最后一条被摘要消息的稳定 message_id，与新 rank 永远同写同读；prep 读阶段校验锚点位置，错位即 agent_run_data_invalid）。invariant：未来新 Run/消息只追加在已覆盖 prefix 之后，prefix 位置永不改变（测试证明）。普通无插队会话中 rank == 物理 seq（与旧测试/数据兼容）。cut 落在完整 exchange 组边界；当前 UserMessage 永不进摘要（through < 当前 user rank）；source_message_count = 最新 marker（累计覆盖原始消息数，稳定语义）。
- **摘要输入边界（不越过模型实际输入）**：渲染绝不整条丢弃 Message——超大 Tool payload 只做单条内部缩写并保留 tool_name/id/is_error/长度省略等语义标记；增量 chunk 超过 Summarizer 输入预算（summarizer_input_max_chars）时按完整 exchange 组从最新端裁减（summarizer_coverable_messages），marker 只推进到实际进入模型输入的最后一条消息（rank+锚点），绝不出现 cut=K 但 K 内部分消息未送达却持久化 through=K。
- **Summarizer 的 cancel/deadline**：Summarizer 调用携带 Run 的 cancel_event/deadline（经 runtime_context → StreamControl）；调用前已取消立即失败，流中取消/超时由 P02 协调器在帧间观察 control 快速中断，不长时间阻塞 context preparation。Runner 在 Summarizer 返回后仍复核 ownership/终态；context_prepared/context_compacted/context_limit 等 Run 事件写入前执行 execution_token fencing。
- **ConversationSummary = monotonic derived cache**：Summary 写（create/conditional update）只允许 through 单调向前（SAVEPOINT 唯一 + expected_old 条件更新），stale worker 写下的摘要只覆盖真实历史 prefix、绝不可能覆盖更新的 winning 值，因此 **Summary write 不需要 execution_token fencing**；Run 生命周期事件仍必须 fencing。
...
- P10.1 新增 conversation_summary（0009，尚未落真实库，命名/锚点在迁移内一次定稿）：每会话一行 current summary，UNIQUE(conversation_id)，through_visible_rank + through_message_id 单调向前（conditional update 带 expected_old 与 <:new 条件，冲突采用 winning 不回退）。并发首建唯一冲突用 SAVEPOINT(begin_nested) 隔离并 expunge failed pending 行——绝不 rollback 调用方外层事务（外层可能同事务写 UserMessage/AgentRun/AgentEvent/Artifact）；MySQL REPEATABLE READ 下以 FOR UPDATE 当前读重取胜者。through/source_message_count 语义见 §3.3（可见逻辑位置 rank/累计覆盖数；prefix 位置稳定性 invariant 有测试）。
