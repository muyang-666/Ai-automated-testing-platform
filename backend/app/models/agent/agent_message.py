...
class AgentMessage(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("session_id", "message_id", name="uq_agent_messages_session_message_id"),
    )
    ...
    message_id = Column(String(64), nullable=True, comment="P01 稳定消息 ID；旧消息可空")
    schema_version = Column(Integer, nullable=False, default=1, comment="P01 消息合同版本")
    timestamp_ms = Column(BigInteger, nullable=True, comment="P01 Unix 毫秒时间戳；旧消息可空")
    role = Column(String(20), nullable=False, comment="角色：user/assistant/toolResult；旧 system/tool 仍可读")
