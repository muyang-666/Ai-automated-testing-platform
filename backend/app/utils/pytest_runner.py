...
def run_pytest_file(file_path: str) -> dict:
    ...
    result = subprocess.run(  # subprocess.run(...) 这是 Python 启动外部命令最常见的方法之一。
        command, # 表示要执行的命令参数列表
        cwd=str(backend_dir),  # 指定子进程的当前工作目录=请在 backend 目录里执行这条 pytest 命令
        capture_output=True,   # 把子进程的标准输出和标准错误都抓回来，不要直接打印到当前终端
        text=True,             # 把子进程输出按文本字符串处理，而不是字节流处理
        encoding="utf-8",      # 指定用 UTF-8 解码。
        errors="replace",      # 如果输出里碰到不能正常按 utf-8 解码的字符，不要直接报错崩掉，而是用替代字符代替。
    )

    # 6.拿到 pytest 输出
    # subprocess.run(...) 返回的是一个 CompletedProcess 对象。它里面通常有这些重要内容：result.stdout：标准输出 result.stderr：标准错误输出
    stdout = result.stdout or ""  # 如果 result.stdout 有内容，就用它，如果是 None 或空，就用空字符串
    stderr = result.stderr or ""  # 同理，处理错误输出。
    ...
    return {
        ...
        "log_content": combined_output, # 这就是完整日志。 结构化字段是摘要，log_content 是原始证据
        # 如果执行成功，就没有错误信息，设为 None。 如果执行失败，就把整段日志也作为错误信息
